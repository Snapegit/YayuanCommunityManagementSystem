	...
	<div class="addEdit-block" :style='{"minHeight":"100vh","padding":"30px 30px 30px 0","fontSize":"14px","color":"#666","background":"#f4f8fb"}'>
		...
		>
			<template >
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","display":"inline-block","width":"100%","fontSize":"inherit"}' class="upload" v-if="type!='info'&& !ro.sheshishipin" label="设施视频" prop="sheshishipin">
					<file-upload
						tip="点击上传设施视频"
						action="file/upload"
						:limit="1"
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","display":"inline-block","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.sheshishipin" label="设施视频" prop="sheshishipin">
					<el-button :style='{"cursor":"pointer","padding":"0 30px","margin":"0 20px 0 0","borderColor":"#e1e8ee","color":"#333","minWidth":"200px","outline":"none","borderRadius":"0px","background":"#f5f8fa","borderWidth":"1px","width":"auto","lineHeight":"32px","fontSize":"14px","borderStyle":"solid","height":"32px"}' type="text" size="small" @click="download($base.url+ruleForm.sheshishipin)">预览</el-button>
				</el-form-item>
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","display":"inline-block","width":"100%","fontSize":"inherit"}' v-else-if="!ruleForm.sheshishipin" label="设施视频" prop="sheshishipin">
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","display":"inline-block","width":"100%","fontSize":"inherit"}' v-if="type!='info'"  label="设施详情" prop="sheshixiangqing">
					<editor 
				...
				<el-form-item :style='{"border":"0px solid #eee","padding":"0","margin":"0 0 20px 0","color":"inherit","borderRadius":"0px","display":"inline-block","width":"100%","fontSize":"inherit"}' v-else-if="ruleForm.sheshixiangqing" label="设施详情" prop="sheshixiangqing">
                    <span :style='{"fontSize":"14px","lineHeight":"40px","minWidth":"1000px","color":"inherit","fontWeight":"500","display":"inline-block"}' v-html="ruleForm.sheshixiangqing"></span>
                </el-form-item>
...
export default {
    ...
    info(id) {
      this.$http({
        ...
        this.ruleForm.sheshixiangqing = this.ruleForm.sheshixiangqing.replace(reg,'../../../springbootqd13b6ty/upload');
        } else {
          this.$message.error(data.msg);
        }
      });
	...
	if(this.ruleForm.sheshishipin!=null) {
		this.ruleForm.sheshishipin = this.ruleForm.sheshishipin.replace(new RegExp(this.$base.url,"g"),"");
	}
var objcross = this.$storage.getObj('crossObj');
				...
				this.$http({ 
					...
					if (data && data.code === 0) { 
						if(data.data.total>=crossoptnum) {
							...
							this.$http({
								url: `gonggongsheshi/${!this.ruleForm.id ? "save" : "update"}`,
								method: "post",
								data: this.ruleForm
							}).then(({ data }) => {
								if (data && data.code === 0) {
									this.$message({
										...
										onClose: () => {
											...
											this.parent.gonggongsheshiCrossAddOrUpdateFlag = false;
											this.parent.search();
    ...
    sheshishipinUploadChange(fileUrls) {
	    this.ruleForm.sheshishipin = fileUrls;
    },
  }
};
</script>
