<template>
	<div class="main-content" :style='{"minHeight":"100vh","padding":"0px 0px 20px","overflow":"hidden","color":"#666","background":"#f4f8fb","fontSize":"14px","position":"relative"}'>
		...
		<template v-if="showFlag">
			<el-form class="center-form-pv" :style='{"border":"0px solid #bababa","padding":"20px 20px 20px 0","margin":"0px 0 0","overflow":"hidden","borderRadius":"0px","background":"none","width":"auto","fontSize":"inherit"}' :inline="true" :model="searchForm">
				...
				<el-row class="actions" :style='{"padding":"0px 20px 10px","margin":"0","borderColor":"#4285f430","color":"#fff","textAlign":"left","flexDirection":"row","display":"block","flexWrap":"wrap","background":"#4285f410","borderWidth":"0 1px 1px","width":"100%","fontSize":"inherit","borderStyle":"solid"}'>
					...
					<el-button class="btn18" v-if="isAuth('tousuxinxi','每日投诉次数')" type="success" @click="chartDialog1()">
				...
				@selection-change="selectionChangeHandler">
					...
					<el-table-column width="300" label="操作">
						<template slot-scope="scope">
							...
							<el-button class="btn8" v-if="isAuth('tousuxinxi','投诉处理')" @click="tousuchuliCrossAddOrUpdateHandler(scope.row,'cross','是','','[1]','该投诉已处理')" type="success">
		...
		<el-dialog
		  ...
		  width="800">
			<div id="tousuriqiChart1" style="width:100%;height:600px;"></div>
		  <span slot="footer" class="dialog-footer">
...
import tousuchuliCrossAddOrUpdate from "../tousuchuli/add-or-update";
	export default {
		...
		methods: {
			...
			tousuchuliCrossAddOrUpdateHandler(row,type,crossOptAudit,crossOptPay,statusColumnName,tips,statusColumnValue){
...
//统计接口
    chartDialog1() {
      ...
      this.$nextTick(()=>{
        var tousuriqiChart1 = echarts.init(document.getElementById("tousuriqiChart1"),'macarons');
        this.$http({
            ...
            if (data && data.code === 0) {
                ...
                // 使用刚指定的配置项和数据显示图表。
                tousuriqiChart1.setOption(option);
                  //根据窗口的大小变动图表
                window.onresize = function() {
                    tousuriqiChart1.resize();
                };
            }
        });
      })
