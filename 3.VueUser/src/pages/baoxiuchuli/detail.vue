...
<div>
	<div :style='{"padding":"20px 0","margin":"0px auto","borderColor":"#ddd","borderRadius":"0px","background":"none","borderWidth":"0 0 0px","width":"86%","borderStyle":"solid"}' class="breadcrumb-preview">
		<el-breadcrumb :separator="'>'" :style='{"fontSize":"14px","lineHeight":"1","justifyContent":"flex-start","display":"flex"}'>
			...
			<el-breadcrumb-item class="item2" v-for="(item, index) in breadcrumbItem" :key="index" to="/index/baoxiuchuli"><a>{{item.name}}</a></el-breadcrumb-item>
	...
	<div class="detail-preview" :style='{"padding":"20px 7%","margin":"0 auto","alignItems":"flex-start","flexWrap":"wrap","background":"#fff","display":"flex","width":"100%","position":"relative","justifyContent":"space-between"}'>
		<div class="attr" :style='{"padding":"0","margin":"0","flexWrap":"wrap","background":"none","flex":"1","display":"flex","width":"48%","position":"relative","justifyContent":"space-between","order":"2"}'>
			...
			<div class="info" :style='{"minHeight":"460px","padding":"0","margin":"0","borderRadius":"0px","flexWrap":"wrap","background":"#dff4ff","display":"flex","width":"100%","justifyContent":"space-between","order":"2"}'>
				...
				<div class="item" :style='{"padding":"0 20px","margin":"0 0 4px 0","borderColor":"#28890b10","background":"none","borderWidth":"0px","display":"flex","width":"48%","borderStyle":"solid","justifyContent":"spaceBetween"}'>
					<div class="lable" :style='{"padding":"0 10px","color":"#333","borderRadius":"20px","textAlign":"center","background":"#fff","width":"auto","fontSize":"13px","lineHeight":"24px","minWidth":"80px","fontWeight":"500","height":"24px"}'>处理日期</div>
					<div  :style='{"padding":"0px 10px 0","fontSize":"14px","lineHeight":"24px","color":"#333","flex":"1","height":"auto"}'>{{detail.chuliriqi}}</div>
				</div>
				<div class="btn" :style='{"width":"100%","padding":"0 20px","flexWrap":"wrap","display":"flex"}'>
					<el-button :style='{"border":"0px solid #009cf530","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#009cf5","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('baoxiuchuli','修改')" @click="editClick">修改</el-button>
					<el-button :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#f53400","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('baoxiuchuli','删除')" @click="delClick">删除</el-button>
					<el-button :style='{"border":"0px solid #f5340030","cursor":"pointer","padding":"0 10px","margin":"0 5px 0 0","outline":"none","color":"#fff","borderRadius":"2px","background":"#f53400","width":"auto","lineHeight":"36px","fontSize":"14px","height":"36px"}' v-if="btnAuth('baoxiuchuli','私聊')&&detail.id!=mid" @click="chatClick">联系TA</el-button>
				</div>
			</div>
		</div>
		...
		<el-tabs class="detail" :style='{"border":"1px solid #ddd","minHeight":"250px","boxShadow":"none","padding":"0","margin":"20px auto","borderRadius":"0px","background":"#fff","display":"block","width":"100%","order":"40"}' v-model="activeName" type="border-card">
			<el-tab-pane label="处理描述" name="first">
				<div v-html="detail.chulimiaoshu"></div>
			</el-tab-pane>
		</el-tabs>
	</div>
  ...
  export default {
    ...
    methods: {
          ...
          this.$http.get(this.tablename + '/detail/'  + this.id, {}).then(res => {
            if (res.data.code == 0) {
              this.detail = res.data.data;
				...
				this.detailBanner = this.detail.weixiutupian ? this.detail.weixiutupian.split(",") : [];
				this.$forceUpdate();
      ...
      nextClick(page) {
		...
		// 修改
		editClick(){
			this.$router.push(`/index/baoxiuchuliAdd?type=edit&&id=${this.detail.id}`);
		},
		// 删除
		async delClick(){
			await this.$confirm('是否删除此报修处理？')
			  .then(_ => {
			    this.$http.post('baoxiuchuli/delete', [this.detail.id]).then(async res => {
					if (res.data.code == 0) {
						this.$message({
							type: 'success',
							message: '删除成功!',
